...
IF NOT [%1] == [] (
  ...
  echo Installing.
  ::Made by https://github.com/ksor-io with <3.
  IF EXIST "%settingsFile%" (
    ...
    move "%settingsFile%" "%USERPROFILE%\AppData\Local\Temp\NotesCompressor\" >NUL 2>&1
  ...
  IF DEFINED restore_settings (
    ...
    move "%USERPROFILE%\AppData\Local\Temp\NotesCompressor\settings.txt" "%installDir%\" >NUL 2>&1
    rmdir /Q /s "%USERPROFILE%\AppData\Local\Temp\NotesCompressor"
  ) ELSE (
...
IF "%fileSize%" LEQ "%thresholdInBytes%" (
  ...
  echo Compressing...
  gswin64c.exe -q -dNOPAUSE -sDEVICE=pdfwrite -sOutputFile=%1\..\%~n1-compreesed.pdf %1 -dBATCH
  
  ::only way to check a file's size I could find is to load it in like so
  echo %1\..\%~n1-compreesed.pdf > %tempFolder%\compreesedFileLocation.txt
  FOR /F %%i IN (%tempFolder%\compreesedFileLocation.txt) DO (
    IF %fileSize% LSS %%~zi (
...
magick "%tempFolder%\temp-images\page-*.jpeg" "%1\..\%~n1-compreesed.pdf"

echo "%1\..\%~n1-compreesed.pdf" > "%tempFolder%\compreesedFileLocation.txt"
FOR /F %%i IN ("%tempFolder%\compreesedFileLocation.txt") DO (
  IF %%~zi LSS %thresholdInBytes% (
    echo File compreesed.
    rmdir /Q /s "%tempFolder%"
    EXIT
  )
  ELSE (
